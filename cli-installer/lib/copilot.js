 ...
 */
async function installCopilotSkills(cacheDir, skills = null, quiet = false) {
  const targetDir = getUserSkillsPath('copilot');
  ...
  for (const skill of skillsToInstall) {
    ...
    try {
      ...
      if (!quiet) console.log(chalk.green(`  ✓ Copilot: ${skill}`));
      installed++;
    } catch (err) {
